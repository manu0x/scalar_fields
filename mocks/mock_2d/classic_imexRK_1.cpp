...
void initialise(fftw_complex *psi,double *k,double dx,int N)
{
	...
	{	di = (double)i;
		...
		{	dj = (double)j;
				...
				ci = i*N+j;
				dci = (double)ci;
				psi[ci][0] = sin(2.0*pie*n*di*dx+2.0*pie*n*dj*dx);
	    		psi[ci][1] = 0.0;
...
/////////////////////////////////////////RK things/////////////////////////////////////////
	...
	for(i=0;i<2;++i)
	{
		...
		for(j=0;j<ex_s;++j)
		{im_K_psi[i][j] =  new double[N2];
		 ex_K_psi[i][j] =  new double[N2];
		}
...
////////////////////////////// Psi variables  /////////////////////////////////////////////////////
	...
	fftw_complex *psi = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * (N2));
	...
	for(t=t_start,tcntr=0;(t<=t_end)&&(!fail)&&( 1);t+=dt,++tcntr)
	{
		...
		//	printf("i,j,k  %d %d %d\n",ii,jj,kk);
			if((tcntr%printcntr)==0||(stb_any))
			{
			  amp  = sqrt(psi[i][0]*psi[i][0] + psi[i][1]*psi[i][1]);
			   ...
			   sol[0] = cos(2.0*pie*t/T)*sin(2.0*pie*n*dx*dbi+2.0*pie*n*dx*dbj);
			   sol[1] = -sin(2.0*pie*t/T)*sin(2.0*pie*n*dx*dbi+2.0*pie*n*dx*dbj);
			 // if(printfp)
			...
			lambda = (k_grid[ii]*k_grid[ii]+k_grid[jj]*k_grid[jj])*im_a[0][0]*dt/(2.0*m*2.0);
			...
			fpGpsi_ft[i][0] = fpGpsi_ft[i][0]/((double)N2);
			fpGpsi_ft[i][1] = fpGpsi_ft[i][1]/((double)N2);

			K_ft[i][0] = -fpGpsi_ft[i][0]*((k_grid[ii]*k_grid[ii]+k_grid[jj]*k_grid[jj]));
			K_ft[i][1] = -fpGpsi_ft[i][1]*((k_grid[ii]*k_grid[ii]+k_grid[jj]*k_grid[jj]));
		...
		*/
		...
		for(s_cntr=1;s_cntr<imex_s;++s_cntr)
		{
			   ...
			   for(i=0;i<N2;++i)
			   {
				...
				/*	if(s_cntr==1)
					...
					*/
					...
					im_K_psi[0][s_cntr-1][i] = -K[i][1]/(2.0*m*2.0);  im_K_psi[1][s_cntr-1][i] =  K[i][0]/(2.0*m*2.0);
			...
			for(i=0,ii=-1,jj=0;i<N2;++i,++jj)
			{
				...
				lambda = (k_grid[ii]*k_grid[ii]+k_grid[jj]*k_grid[jj])*im_a[s_cntr][s_cntr]*dt/(2.0*m*2.0);
				//if(lambda>1.0)
		...
		*abs_err = 0.0;
		...
		for(i=0,ii=-1,jj=0;i<N2;++i,++jj)
		{
			...
			im_K_psi[0][imex_s-1][i] = -K[i][1]/(2.0*m*2.0);  im_K_psi[1][imex_s-1][i] =  K[i][0]/(2.0*m*2.0);
			...
			sol[0] = cos(2.0*pie*(t+dt)/T)*sin(2.0*pie*n*dx*dbi+2.0*pie*n*dx*dbj);
			sol[1] = -sin(2.0*pie*(t+dt)/T)*sin(2.0*pie*n*dx*dbi+2.0*pie*n*dx*dbj);
