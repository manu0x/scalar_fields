...
double potn_vel_eqn(double a,double a_t,double phi,double lap_phi,double f_a,double omega_dm_0,double Xb_0,double a0=1.0)
{     
	...
	potn_t = -phi/(a) + lap_phi/(3.0*a_t*a_t*a) + 2.0*twopie*G*a*(2.0*alpha-1.0)*x_power_approx*((Xb)/(a_t*a_t)-0.5*f_a*f_a/(1.0+2.0*phi))/(3.0);
