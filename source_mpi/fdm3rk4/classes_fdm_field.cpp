...
class metric_potential_poisson_mpi
{
	...
	void solve_poisson(double **k_grid,int krk,double a,double a_t,double da)
	{
		...
		for(i=0;i<n_loc[0];++i)
		{
		  for(j=0;j<n_loc[1];++j)
		  {
			...
			if(k2fac>0.0)
			{
				...
				if(method==0)
				{fpGpsi_ft[ci][0] = fpGpsi_ft[ci][0]/(1.0+rk[krk]*da*(c_box*100000.0/a_t)*(c_box*100000.0/a_t)*k2fac/(3.0*a));
			 	 fpGpsi_ft[ci][1] = fpGpsi_ft[ci][1]/(1.0+rk[krk]*da*(c_box*100000.0/a_t)*(c_box*100000.0/a_t)*k2fac/(3.0*a));
				}
...
class fdm_poisson_mpi
{
	...
	fdm_poisson_mpi(int *ind,int *ind_loc,int cum_lin_ind_ar,bool lb=false,bool sgb=false):psi_ms(ind_loc,cum_lin_ind_ar,lb,sgb)
	{
		...
		s_psi =  new double [ind_loc[0]*ind_loc[1]*ind_loc[2]*2];
	...
	void solve_poisson(double **k_grid,double Xb[2],int krk,double a,double a_t,double da)
	{
		int i,j,k,ci,ind[3]{0,0,0},r;
		...
		for(i=0;i<n_loc[0];++i)
		{
		  for(j=0;j<n_loc[1];++j)
		  {
			...
			lambda = 0.5*rk[krk]*da*k2fac*hbar_by_m/(a*a*a_t);
	...
	void update_A(int ci,double potn,double f_r,double f_i,int krk,double a,double a_t,double da)
	{
		...
		if(method==0)		
		{fpGpsi[ci][0] = f_r + rk[krk]*da*potn*fdm_v_i/(hbar_by_m*a_t);//Conv. from phi->phi_c has been done...
		 fpGpsi[ci][1] = f_i - rk[krk]*da*potn*fdm_v_r/(hbar_by_m*a_t);
		}		


		if(method==1)		
		{fpGpsi[ci][0] = fdm_v_r +  rk[krk]*da*potn*(c_box*100000.0)*(c_box*100000.0)*fdm_v_i/(hbar_by_m*a*a_t);
									//Conv. from phi->phi_c has been done...But now it already phi_c so conversion not req. here
		 fpGpsi[ci][1] = fdm_v_i -  rk[krk]*da*potn*(c_box*100000.0)*(c_box*100000.0)*fdm_v_r/(hbar_by_m*a*a_t);
		}
