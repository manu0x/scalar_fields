...
double run(double dt,int N,double *mass_err,int argc,char **argv,int prntfp,int prnt)
{
	...
	int t_steps;
	double box_len,t,a,t_end,t_start,tk,dt_up,xval,dbi,dbj,dbk,dx;
...
*/
		...
		{	
			...
			lambda = ksqr*imx.im_a[0][0]*psi_1.kppa*dt/(a*a*2.0);
			//printf("lmda  %lf\n",lambda);
		...
		//		da_up = 0.7*imx.ex_stb_r/vmax;
			...
			for(i=0,ii=-1,jj=-1,kk=0;i<N3;++i,++kk)
			{
				...
				ksqr = k_grid[ii]*k_grid[ii] + k_grid[jj]*k_grid[jj] + k_grid[kk]*k_grid[kk];
			 	...
			 	lambda = ksqr*imx.im_a[s_cntr][s_cntr]*psi_1.kppa*dt/(a*a*2.0);
		...
		for(i=0,ii=-1,jj=-1,kk=0;i<N3;++i,++kk)
		{
			...
			a = psi_1.aoft(tk);
			psi_1.ex_rhs(i,imx.s-1,a);

			
			tk = t+imx.im_c[imx.s-1]*dt;
			a = psi_1.aoft(tk);
			psi_1.im_rhs(i,imx.s-1,a*a);
