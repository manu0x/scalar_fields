 ...
 #pragma omp parallel for private(j,k,ci,ind,c1,fa_k,fa_vel,potn,potn_k,potn_t,acc_fa,potn_der)
	 for(i=0;i<n[0];++i)
	 {
		  for(j=0;j<n[1];++j)
		  {
			...
			c1 = f_alpha.calc_acc(ind,acc_fa, potn_val[ci], potn_t,potn_der, a, a_t,a_tt,dx);
	...
	for(i=0;(i<n[0])&&(!fail);++i)
	 {
		  for(j=0;(j<n[1]);++j)
		  {
			...
			c1 = phi.get_potn_spt_der(ind,potn_der);	
			c1 = f_alpha.calc_acc(ind,acc_fa, potn_k, potn_t,potn_der, ak, a_t,a_tt,dx);
			...
			potn_k = 0.5*(potn_k+potn_val[ci]+potn_t*da);
