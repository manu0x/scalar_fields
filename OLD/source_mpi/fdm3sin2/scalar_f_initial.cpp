	...
	for(i=0;i<n[0];++i)
		{
		  for(j=0;j<n[1];++j)
		  {
			...
			potn = 1.0 - twopie*M_PI*nwave_amp*nwave_amp/(alpha*alpha);
			
			dot = nwave[0]*x_grid[ci][0] + nwave[1]*x_grid[ci][1] + nwave[2]*x_grid[ci][2];
			
			
			
			psi_val[0] = sin(twopie*dot);
			psi_val[1] = 0.0;
			...
			fprintf(fpstoreini,"%.10lf\t%.10lf\t%.10lf\t%.10lf\t%.15lf\t%.15lf\n",
							a,dx[0]*i,dx[1]*j,dx[2]*k,potn,psi_val[0]);
